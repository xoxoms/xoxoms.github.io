...
또 객체지향 언어를 사용하면서도 SQL 중심의 개발을 해야하기 때문에 객체지향언어의 장점을 살리기 힘들어 단순히 테이블에 맞춰 데이터를 전달하는 역할만 할 수 도 있다. 이렇게 객체지향 언어를 사용해서 SQL 중심의 개발을 할 때 나타나는 문제점들을 패러다임의 불일치라고 하는데 JPA는 객체와 테이블의 관계를 잘 매핑해준다면 패러다임의 불일치를 효과적으로 해결해준다.